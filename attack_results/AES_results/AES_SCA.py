...
class AES_SCA(object):
    ...
    def __init__(self):
        ...
        fc = 2000 # Filter CutOff frequency (KHz)
        order = 4 # Filter order
        w = fc / (fs / 2) # Normalize the frequency
        self.b,self.a = signal.butter(order, w , btype='highpass',output='ba') # HP butterworth filter
        ...
        ##### CPA accumulators #####
        self.sumAvg      = np.zeros(self.nSample,dtype=np.double)
        self.sumPopClass = np.zeros((self.nByte,self.nClass),dtype=np.uint32)
        self.sumVar      = np.zeros(self.nSample,dtype=np.double)
        self.sumAvgClass = np.zeros((self.nByte,self.nClass,self.nSample),dtype=np.double)
        self.sumVarClass = np.zeros((self.nByte,self.nClass,self.nSample),dtype=np.double)

        ##### Create a folder to save CPA results #####
        counterlog = 0
        try:
    ...
    def Data_Acquisition(self):
        ...
        self.sumVar =  np.divide(self.sumVar,self.nTrace) - np.square(self.sumAvg)

        ####### SAVE AVERAGE VALUE png AND BIN #######
        ...
        plt.xlabel("Time (samples)")
        plt.ylabel("Quantification")
        plt.plot(self.sumAvg,linewidth=0.7, linestyle="-")
    ...
    def Compute_Correlation(self,iByte):

        #Turn Global variance into standard deviation
        Correlation = np.zeros((self.nClass,self.nSample),dtype=np.double)
        hammingDistance = np.zeros(self.nClass,dtype=np.double)

        #Compute Class Variance & Average and save it in binary files
        for iClass in range(0, self.nClass):
            if(self.sumPopClass[iByte][iClass]>0):
                self.sumAvgClass[iByte][iClass] = np.divide(self.sumAvgClass[iByte][iClass],self.sumPopClass[iByte][iClass])
        ...
        for iHyp in range(0, self.nHyp):
            ...
            for iClass in range(0,self.nClass): 
                ...
                if iByte == 0  or iByte == 5 or iByte == 10 or iByte == 15:
                    vClass = self.Te0[vClass]
                elif iByte == 4 or iByte == 9 or iByte == 14 or iByte == 3:
                    vClass = self.Te1[vClass]
                elif iByte == 8 or iByte == 13 or iByte == 2 or iByte == 7:
                    vClass = self.Te2[vClass]
                else:
                    vClass = self.Te3[vClass]

                hammingDistance[iClass] = bin(vClass).count("1")

                DY                  += np.multiply(self.sumPopClass[iByte][iClass],hammingDistance[iClass])
                YVAR                += np.multiply(self.sumPopClass[iByte][iClass], np.square(hammingDistance[iClass]))

            DY = np.divide(DY,self.nTrace)          
            YVAR = np.divide(YVAR,self.nTrace) - np.square(DY) 
            YSTDDEV = np.sqrt(YVAR)

            DX = np.zeros(self.nSample)
            XVAR = np.zeros(self.nSample)
            DXY = np.zeros(self.nSample)

            #Classification stats
            for iClass in range(0,self.nClass):
                #Acumulator for sum of products
                DXY     += np.multiply(self.sumPopClass[iByte][iClass],np.multiply(self.sumAvgClass[iByte][iClass],hammingDistance[iClass]))

            #Covariance : average of product minus product of averages
            Correlation[iHyp] = np.divide(DXY,self.nTrace) - np.multiply(self.sumAvg,DY)
    ...
    def Save_Correlation_Results(self,Correlation,iByte):

        currentMax = 0.0
        currentArgMax = 0
        hyprank = np.zeros(256)
        minArray = np.full(len(Correlation[0]),1000000.0)
        maxArray = np.full(len(Correlation[0]),-1000000.0)

        for iClass in range(0,self.nClass):
            if np.amax(abs(Correlation[iClass])) > currentMax:
                currentArgMax = iClass
                currentMax = np.amax(abs(Correlation[iClass]))

        for iClass in range(0,self.nClass):
            for iSample in range(0, self.nSample):

                if (Correlation[iClass][iSample] > maxArray[iSample]) and iClass != currentArgMax:
                    maxArray[iSample] = Correlation[iClass][iSample]

                if (Correlation[iClass][iSample] < minArray[iSample]) and iClass != currentArgMax:
                    minArray[iSample] = Correlation[iClass][iSample]
        ...
        if(self.keyArray[iByte] == currentArgMax):
            ...
            plt.plot(Correlation[currentArgMax],linewidth=0.5, linestyle='dotted',color='r')
            plt.plot(Correlation[self.keyArray[iByte]],linewidth=0.7, linestyle="-",color='g')

        plt.fill_between(range(0,len(Correlation[0])),minArray, maxArray,color='#C0C0C0')
        ...
        plt.savefig(os.path.join(self.outFolder,"corr_%02d.png"%(iByte)),dpi=600,transparent=True,bbox_inches='tight') 
        plt.close('all')
...
##### Acquire Data and compute CPA accumulators #####
cd.Data_Acquisition()

##### Compute CPA #####
for iByte in range(0,cd.nByte):
    Correlation = cd.Compute_Correlation(iByte)
    cd.Save_Correlation_Results(Correlation,iByte)